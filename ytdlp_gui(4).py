...
class YtDlpGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Python 進階影音下載器 (yt-dlp) - Windows 相容版")
    ...
    def on_mode_change(self):
        ...
        if mode == "video":
            ...
            self.format_combo.current(0) # 預設 mp4
            if self.is_analyzed:
                self.video_combo.config(state="readonly")
        elif mode == "audio":
            self.format_label.config(text="音檔格式:")
            # 修改點：將 m4a 設為第一位並作為預設
            self.format_combo.config(values=["m4a", "mp3", "flac", "opus", "aac", "wav"])
    ...
    def analyze_url(self, url):
        ...
        try:
            ...
            # 修改點：'ba' 加上擴充過濾，預設優先找 m4a
            self.video_formats = [{'id': 'bv', 'desc': 'Best Video'}]
            self.audio_formats = [{'id': 'ba[ext=m4a]/ba', 'desc': '最佳音質 (優先 m4a)'}]
            
            v_options = ["最高畫質 (自動選擇)"]
            a_options = ["最高音質 (優先 m4a)"]
    ...
    def start_download_thread(self):
        ...
        ydl_opts = {
            ...
            'writethumbnail': True,          # ← 新增縮圖
        }

        if mode == "video":
            v_idx = self.video_combo.current()
            v_id = self.video_formats[v_idx]['id']
            # 修改點：在合併影音時，若選「自動」，強制指定音軌優先權
            if a_id == 'ba[ext=m4a]/ba':
        ...
        self.status_label.config(text=f"開始下載...")
        self.download_btn.config(state="disabled")
    ...
    def download_video(self, opts):
        ...
        try:
            ...
            self.root.after(0, lambda: messagebox.showinfo("成功", "檔案處理完成。"))
        except Exception as e:
    ...
    def progress_hook(self, d):
        if d['status'] == 'downloading':
            ...
            self.root.after(0, lambda: self.status_label.config(text="正在轉碼/合併 (FFmpeg)..."))
